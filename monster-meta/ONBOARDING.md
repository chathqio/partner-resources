# monster-meta - plugin onboarding runbook

This is the **monster-meta** plugin's own setup runbook. The marketplace orchestrator (the repo
root `CLAUDE.md`) delegates here once the user chooses to install monster-meta. Your job: get the
...
This runbook is **resumable**. Installing the MCP requires a restart, after which the root runbook
re-runs and returns here. So on every session, first detect what's already done and continue from
there - never redo a completed step.
